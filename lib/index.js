...
const { MapControls } = require('prismarine-viewer/viewer/lib/controls')
...
let controls = new MapControls(viewer.camera, renderer.domElement)
...
socket.on('version', (version) => {
    ...
    socket.on('position', ({ pos, addMesh, yaw, pitch }) => {
        ...
        if (pos.y > 0 && firstPositionUpdate) {
            controls.target.set(pos.x, pos.y + 1.62, pos.z)
            viewer.camera.position.set(pos.x, pos.y, pos.z + 0.000001)
            controls.update()
